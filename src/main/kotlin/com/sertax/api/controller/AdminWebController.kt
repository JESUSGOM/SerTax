...
class AdminWebController(
    ...
    fun showDashboardPage(model: Model): String {
        model.addAttribute("dashboardStats", adminDataService.getDashboardStats())
        model.addAttribute("pendingTrips", adminDataService.getPendingManualTrips())
    ...
    // --- GESTIÓN DE USUARIOS ---
    @GetMapping("/users")
    fun showUsersPage(model: Model): String {
    ...
    fun saveUser(@ModelAttribute("userDto") userDto: CreateUserRequestDto, redirectAttributes: RedirectAttributes): String {
        try {
            adminDataService.createUser(userDto)
    ...
    @GetMapping("/licenses")
    fun showLicensesPage(model: Model): String {
        model.addAttribute("licenses", adminDataService.getAllLicenses())
        model.addAttribute("pageTitle", "Gestión de Licencias")
        return "admin/licenses"
    }
    
    @GetMapping("/licenses/new")
    fun showCreateLicenseForm(model: Model): String {
    ...
    fun showEditLicenseForm(@PathVariable id: Long, model: Model): String {
        val license = adminDataService.getLicenseById(id)
        ...
        model.addAttribute("associations", adminDataService.getAllAssociations())
        model.addAttribute("pageTitle", "Editar Licencia")
        model.addAttribute("formAction", "/admin/licenses/update/$id")
        return "admin/license-form"
    }
    
    @PostMapping("/licenses/save")
    fun saveLicense(@ModelAttribute("licenseDto") licenseDto: CreateLicenseRequestDto, redirectAttributes: RedirectAttributes): String {
        try {
            adminDataService.createLicense(licenseDto)
    ...
    fun updateLicense(@PathVariable id: Long, @ModelAttribute("licenseDto") licenseDto: UpdateLicenseRequestDto, redirectAttributes: RedirectAttributes): String {
        try {
            adminDataService.updateLicense(id, licenseDto)
    ...
    @GetMapping("/drivers")
    fun showDriversPage(model: Model): String {
    ...
    fun showCreateDriverForm(model: Model): String {
        model.addAttribute("driverDto", CreateDriverRequestDto("", "", DriverRole.Owner, 0L, true))
        model.addAttribute("licenses", adminDataService.getAllLicenses())
        model.addAttribute("roles", DriverRole.values())
        model.addAttribute("pageTitle", "Crear Nuevo Conductor")
        model.addAttribute("formAction", "/admin/drivers/save")
        return "admin/driver-form"
    }
    
    @GetMapping("/drivers/edit/{id}")
    fun showEditDriverForm(@PathVariable id: Long, model: Model): String {
        val driver = adminDataService.getDriverById(id)
        ...
        model.addAttribute("currentLicense", driver.license)
        model.addAttribute("roles", DriverRole.values())
        model.addAttribute("pageTitle", "Editar Conductor")
        model.addAttribute("formAction", "/admin/drivers/update/$id")
        return "admin/driver-form"
    }
    
    @PostMapping("/drivers/save")
    fun saveDriver(@ModelAttribute("driverDto") driverDto: CreateDriverRequestDto, redirectAttributes: RedirectAttributes): String {
        try {
            adminDataService.createDriver(driverDto)
    ...
    fun updateDriver(@PathVariable id: Long, @ModelAttribute("driverDto") driverDto: UpdateDriverRequestDto, redirectAttributes: RedirectAttributes): String {
        try {
            adminDataService.updateDriver(id, driverDto)
    ...
    @GetMapping("/vehicles")
    fun showVehiclesPage(model: Model): String {
    ...
    fun showCreateVehicleForm(model: Model): String {
        model.addAttribute("vehicleDto", CreateVehicleRequestDto("", 0L, null, null, false, false))
        model.addAttribute("licenses", adminDataService.getAllLicenses())
        model.addAttribute("pageTitle", "Registrar Nuevo Vehículo")
        model.addAttribute("formAction", "/admin/vehicles/save")
        return "admin/vehicle-form"
    }
    
    @GetMapping("/vehicles/edit/{id}")
    fun showEditVehicleForm(@PathVariable id: Long, model: Model): String {
        val vehicle = adminDataService.getVehicleById(id)
        ...
        model.addAttribute("formAction", "/admin/vehicles/update/$id")
        return "admin/vehicle-form"
    }
    
    @PostMapping("/vehicles/save")
    fun saveVehicle(@ModelAttribute("vehicleDto") vehicleDto: CreateVehicleRequestDto, redirectAttributes: RedirectAttributes): String {
        try {
            adminDataService.createVehicle(vehicleDto)
    ...
    fun updateVehicle(@PathVariable id: Long, @ModelAttribute("vehicleDto") vehicleDto: UpdateVehicleRequestDto, redirectAttributes: RedirectAttributes): String {
        try {
            adminDataService.updateVehicle(id, vehicleDto)
    ...
    // --- HISTORIAL DE VIAJES ---
    @GetMapping("/trips")
    fun showTripsPage(model: Model): String {
    ...
    fun showIncidentsPage(model: Model): String {
        model.addAttribute("incidents", adminDataService.getAllIncidents())
        model.addAttribute("pageTitle", "Gestión de Incidencias")
        return "admin/incidents"
    }
    
    @GetMapping("/incidents/{id}")
    fun showIncidentDetailPage(@PathVariable id: Long, model: Model): String {
        try {
            model.addAttribute("incident", adminDataService.getIncidentDetails(id))
    ...
    fun updateIncidentStatus(@PathVariable id: Long, @RequestParam("status") newStatus: IncidentStatus, redirectAttributes: RedirectAttributes): String {
        try {
            adminDataService.updateIncidentStatus(id, newStatus)
    ...
    @GetMapping("/config")
    fun showConfigPage(model: Model): String {
        model.addAttribute("configs", adminDataService.getAllSystemConfigs())
        model.addAttribute("pageTitle", "Configuración del Sistema")
        return "admin/config"
    }
    
    @PostMapping("/config/update")
    fun updateConfig(@RequestParam("configKey") key: String, @RequestParam("configValue") value: String, redirectAttributes: RedirectAttributes): String {
        try {
            val request = UpdateSystemConfigRequestDto(value)
            adminDataService.updateSystemConfig(key, request)
            redirectAttributes.addFlashAttribute("successMessage", "Parámetro '$key' actualizado con éxito.")
        } catch (e: Exception) {
            redirectAttributes.addFlashAttribute("errorMessage", "Error al actualizar el parámetro: ${e.message}")
        }
    ...
    fun processManualAssign(
        @RequestParam("tripId") tripId: Long,
        @RequestParam("driverId") driverId: Long,
        principal: Principal,
        redirectAttributes: RedirectAttributes
    ): String {
        try {
            adminDataService.assignDriverManually(tripId, driverId, principal.name)
            redirectAttributes.addFlashAttribute("successMessage", "Taxi asignado correctamente al viaje #$tripId.")
        } catch (e: Exception) {
            redirectAttributes.addFlashAttribute("errorMessage", "Error al asignar el taxi: ${e.message}")
        }
